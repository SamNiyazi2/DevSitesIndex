...
export class DashboardComponent implements OnInit {
    ...
    setup_Timelog_SummaryByWeekAndDiscipline() {
        ...
        this.dataService.getTimelog_SummaryByWeekAndDiscipline().subscribe((data: any[]) => {
            ...
            weekEndingList.forEach((weekEndingDate) => {


                let dataSource = data.filter(r2 => r2.weekEnding === weekEndingDate);
                ...
                weekendingArray.push(weekEndingDate);

            });

            disciplineList.forEach((_discipline1) => {

                let disciplinesHoursDataSource = data.filter(r2 => r2.disciplineShort == _discipline1);

                let disciplineTotalHours = disciplinesHoursDataSource.reduce(function (previousValue: number, currentObj: any, ndx: number) {
                    return previousValue + currentObj.totalHours;
                }, 0);

                let _label = disciplineTotalHours.toFixed(2) + " - " + _discipline1;

                let _data = disciplinesHoursDataSource.map(r3 => (r3.totalHours == null) ? 0 : r3.totalHours);

                dataSets.push(
                    {
                        // 07/16/2022 08:57 pm - SSN - [20220716-2034] - [003] - Chart height ratio
                        //barPercentage: 0.9,
                        barThickness: 2,
                        //maxBarThickness: 8,
                        minBarLength: 0,
